...
class BlogWriter:
    ...
    def gen_prompt_title(self, text_body=None):
        ...
        return f"Write a title in strictly maximum 10 words in Korean from all Korean text below\n{text_body}"

    def gen_prompt_tags(self, text_body=None):
        return f"Generate a hashtag (or a keyword) in Korean from all Korean text below\n{text_body}"

    def get_data(self, prompt_str):
        return {
            "model": "gpt-4",
            "messages": [
            ...
            "max_tokens": 1000,  # Optional: Limit the response length
